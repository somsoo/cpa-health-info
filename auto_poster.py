...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["하이스트 비뇨기과 5S 복합 남성확대수술", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 남성의학 및 비뇨의학과 전문 임상 헬스케어 디렉터입니다.
...
{{
  ...
  "thumb_hook": "{keyword}\\n핵심 실무 가이드 (줄바꿈 \\n)",
  "vibe_keywords": "doctor hospital health",
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'doctor hospital health')
